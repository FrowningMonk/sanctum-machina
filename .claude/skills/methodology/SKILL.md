...
- Researches codebase and checks dependencies; external library docs via WebFetch/WebSearch when needed
...
### Step 4: Implementation — `/do-task`

Single task per session. Manual, controlled execution.
...
**New feature:**
`/new-user-spec` → `/new-tech-spec` → `/decompose-tech-spec` → `/do-task` → `/done`
