...
**ErrorLog component strings are a closed whitelist.** Only these values are allowed as the `component` argument to `ErrorLog.e(component, description, cause?)`: `"download"`, `"inference-init"`, `"inference"`, `"inference-cleanup"`. Adding a new component requires updating the whitelist in the reviewing spec (currently Phase 1 tech-spec TAC-9) and the code audit. This keeps the error log grep-friendly and bounded.
