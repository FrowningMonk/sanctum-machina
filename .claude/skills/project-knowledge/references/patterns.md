...
Phase-origin breakdown of the whitelist: the first 4 (`download`, `inference-init`, `inference`, `inference-cleanup`) shipped in Phase 1. `settings-io` was added in Phase 2.5 with the per-model settings DataStore. `camera`, `audio`, `attachment-decode` were added in Phase 2 (multimodal input). The remaining 6 (`model`, `engine-warmup`, `history-read`, `history-write`, `attachment-save`, `attachment-read`) are Phase 3 additions: `model` covers registry-level init failures beyond inference-init, `engine-warmup` covers `WarmupCoordinator` lifecycle failures, the `history-*` pair covers Room chat/message read/write failures, and the `attachment-save`/`-read` pair covers the Phase 3 disk staging and decode-on-read paths for images and audio.
