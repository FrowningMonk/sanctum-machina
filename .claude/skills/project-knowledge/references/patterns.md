...
**ErrorLog component strings are a closed whitelist, enforced at runtime.** Only these 15 values are allowed as the `component` argument to `ErrorLog.e/i/w(component, description, cause?)`: `"download"`, `"inference-init"`, `"inference"`, `"inference-reset"`, `"inference-cleanup"`, `"settings-io"`, `"camera"`, `"audio"`, `"attachment-decode"`, `"model"`, `"engine-warmup"`, `"history-read"`, `"history-write"`, `"attachment-save"`, `"attachment-read"`. Unknown component → `IllegalArgumentException` thrown at call site (covered by `ErrorLogTest`). Adding a new component requires updating the constant set in `ErrorLog.kt` AND this doc AND the reviewing spec. Rationale: grep-friendly, bounded taxonomy, mapped one-to-one to specific failure modes.

Phase-origin breakdown of the whitelist: the first 4 (`download`, `inference-init`, `inference`, `inference-cleanup`) shipped in Phase 1. `settings-io`, `camera`, `audio`, `attachment-decode` were added in Phase 2 (multimodal input + per-model settings DataStore — see Decision D27). The next 6 (`model`, `engine-warmup`, `history-read`, `history-write`, `attachment-save`, `attachment-read`) are Phase 3 additions: `model` covers registry-level init failures beyond inference-init, `engine-warmup` covers `WarmupCoordinator` lifecycle failures, the `history-*` pair covers Room chat/message read/write failures (and the `di/AppModule.provideSanctumDatabase` corruption-recovery path), and the `attachment-save`/`-read` pair covers the Phase 3 disk staging and decode-on-read paths for images and audio. Phase 3.6 added `inference-reset` (see next paragraph).

`inference-reset` (Phase 3.6 addition) tags every `DefaultModelRegistry.resetConversation` call regardless of caller, with the `description` payload encoding which `ResetReason` triggered it — one of `CHAT_SWITCH` (drawer chat handover), `DRAFT_COMMIT` (first send in a Quick/Draft promoted to Persistent), `LIGHT_OVERRIDE` (sampler-only Apply from Settings sheet), `SYSTEM_PROMPT` (semi-light system-prompt change), `HEAVY` (post-reinit reset, if any), or `USER` (explicit ↻ tap). Level convention: `errorLog.i` on the success path (helper invoked under `lifecycleMutex`), `errorLog.w` on the non-Ready skip (engine still `Idle` / `Initializing` / `Failed` — caller now has a paper trail instead of a silent return). Success-path payload also carries `replayedMessages=N` (Phase 3.6 Task 11) — the count of `ConversationConfig.initialMessages` rows used to prefill the recreated Conversation; non-zero on CHAT_SWITCH and on Persistent LIGHT_OVERRIDE, zero on every other reason. Operational role: surfaces in the SAF-exported diagnostic log (Diagnostics → Export logs) so KV-cache leakage between chats, stale-sampler regressions, and missed-replay regressions can be triaged from a single grep on `inference-reset`.
...
**Edge-to-edge is project-wide; new Activities must opt in (Phase 3.6).** Every Activity with Compose UI calls `enableEdgeToEdge()` (from `androidx.activity:activity-compose`) before `setContent`. Currently `MainActivity.onCreate` and `CrashReportActivity.onCreate` (after `setFlags(FLAG_SECURE)`) — adding a Phase 4 `OnboardingActivity` or similar requires the same opt-in. Compose layer relies on this: `Scaffold(contentWindowInsets = WindowInsets.systemBars)` (M3 default) + `Modifier.consumeWindowInsets(innerPadding).imePadding()` is what `ChatScreen` uses to absorb status / nav / IME insets correctly; without `enableEdgeToEdge()` the system would also apply window-level insets and the column would be pushed down. Empirical note: SwiftKey on Honor 200 reports a larger IME inset than Gboard / system keyboards (extra padding under its toolbar zone) — the visible "gap" between IME and input bar is keyboard-side, not a layout bug; do not chase double-inset symptoms without first checking the active IME.

**SAF log export.** User-facing log export uses `ActivityResultContracts.CreateDocument("text/plain")` with a suggested filename (`sanctum-log-YYYYMMDD-HHmm.txt`, formatted with `Locale.ROOT`). This avoids `FileProvider` registration, URI-permission granting, and a third-party app in the share chain. The caller writes assembled content via `ContentResolver.openOutputStream(uri)`. An in-flight guard flag (`var launching`) prevents launching a second SAF dialog before the first one returns its result — reset in a `finally` block on all three branches (success / IOException / cancel). Phase 3.5: launcher + button live in `DiagnosticsScreen`/`DiagnosticsViewModel` (migrated whole from `AboutScreen`/`AboutViewModel` per Decision 11 — no duplicating composable). `:crash` recovery activity keeps its own SAF launcher unchanged.
...
**Pre-flight model-RAM gate (Phase 3.5).** A model is gateable for download iff `MemoryInfo.totalMem` (read once at VM init via `DeviceInfoProvider.totalMemoryBytes()`) is `>=` its `Model.minDeviceMemoryInGb * 1_073_741_824L`. Comparison is byte-precise — early rounding to GB false-negatives nominal-12-GB devices that report ~11.5 GB. Failure renders a disabled «Скачать» button + Russian secondary line «Недостаточно RAM (X.X ГБ устройство, нужно Y ГБ)»; tap is a no-op (Compose disabled + defence-in-depth early-return in `onDownload`). Gate trip is a preventive UX, **not an incident** — `ErrorLog.ALLOWED_COMPONENTS` is NOT extended for it. Allowlist parser separately rejects records with null or out-of-range `minDeviceMemoryInGb` (1..64) and writes one `ErrorLog.e("download", …)` line per rejected batch (aggregate fail-fast — one bad record drops the whole batch so the registry never carries a partially-trusted allowlist). Calibrations are conservative-empirical (E2B = 4 GB, E4B = 6 GB) — recalibrate in a patch release if false-positive reports come in. Future allowlist additions (e.g., FunctionGemma-270M in Phase 4) MUST set the field explicitly; forgetting it surfaces as a fail-loud rejection in `errors.log`.

**Auto-derived `versionName` (Phase 3.5).** `app/build.gradle.kts` reads `versionName` from the `phonewrap.git-version` convention plugin (`build-logic/`) which runs `git describe --tags --always --dirty=-dev` via config-cache-friendly `providers.exec`. Pure parser `gitVersionParse(stdout, exitCode): String?` is JUnit-tested separately. Fallback hardcoded literal `"v0.3.5-diagnostics-fallback"` kicks in when `.git` is missing (shallow clone, tarball checkout) or exec fails. Tagging a phase no longer requires a manual `versionName` bump — release tags propagate to `BuildConfig.VERSION_NAME` automatically; commits ahead of a tag get a `-N-g<sha>` suffix and dirty workdir gets a `-dev` suffix.

**Logcat capture filter (Phase 3.5).** `LogcatReader` collects `*:W` (was `*:E`) — the predeath WARN entries from LiteRT-LM before native abort would otherwise be lost. The "ERROR level only" rule above governs **app-side `ErrorLog` writes**; logcat capture is a different mechanism (system-level export of foreign processes). The `LogExportManager` 100 KB tail-truncation cap was empirically validated on Honor 200 (Phase 3.5 user smoke: ≈ 3.1 KB block, both WARN and ERROR present, 33× headroom under cap).
...
**Streaming assistant messages:** for persistent chats, Room stores only committed messages — the USER row is written synchronously at send time, the ASSISTANT row only on `done = true` (AC-R2; Phase 3 Decision 4). Mid-stream tokens never hit Room: they accumulate in `ChatViewModel._streamingMessage: StateFlow<Message?>`; the UI layer renders `combine(messageDao.observeByChat(chatId), _streamingMessage)` and the handover clears `_streamingMessage` inside the same `mapLatest` block that observes the newly-persisted ASSISTANT row (prevents double-bubble flicker). Quick chats and drafts stay fully in-memory — `MessageDao` is never called from `ChatIdentity.Quick` or `ChatIdentity.Draft` paths. The thinking channel accumulates separately into `thinkingSb: StringBuilder`, gated once per send on `model.llmSupportThinking && effectiveConfig[ENABLE_THINKING]`; mid-stream config flip cannot half-populate the bubble. Attachments (`Bitmap` / raw PCM `ByteArray`) are snapshotted into `Message.attachments` when the USER message is appended and cleared from `_attachments` immediately after. Interrupted streams (process kill mid-response) leave USER persisted and ASSISTANT lost (AC-R3); on reopen, `ChatViewModel` auto-resumes inference once per unpaired tail USER (Task 18 B4).
...
**Room DAOs are interfaces with suspend/Flow functions only.** No blocking DAO methods. ViewModels collect `Flow<List<T>>` and expose `StateFlow` to Compose. `ChatDao` / `MessageDao` (Phase 3, v1 schema) follow this rule; every write is `suspend`, every read over a set is `Flow<List<T>>`. Multi-entity writes that must be all-or-nothing (commit draft chat, rename attachments dir) are marked `@Transaction` on the DAO method, not orchestrated from the repository.
...
- **Light** (`temperature`, `topK`, `topP`) — override stored in DataStore, `model.configValues` reassigned to `EffectiveConfig.merge(defaults, overrides)`, then `registry.resetConversation(reason = LIGHT_OVERRIDE)` recreates the engine's `Conversation` with the new `SamplerConfig`. Engine is not torn down (no `cleanup + initialize`) and UI history (in-memory message list + Room rows) is preserved — Light is a sampler refresh, not a context wipe. The Apply button is gated by `!isGenerating`, so this path does not arise mid-stream. `maxTokens` is baked into `EngineConfig` at engine creation and therefore lives in the Heavy tier. In Persistent identity the reset also passes `initialMessages` (paired history, mapped via `MultimodalContentsBuilder` → `Contents.of(...)` → `Message.user/model`) so the recreated Conversation is prefilled with the same context the user can still see — same prefill mechanism CHAT_SWITCH uses; without it the slider-Apply would erase context that's still visible on screen (Phase 3.6 Task 11).
...
- **`phase/N-name`** — working branches for each phase: `phase/1-foundation`, `phase/2-ui`, `phase/2.5-logexport`, `phase/3-history`, `phase/3.5-diagnostics` (planned), `phase/4-agent` (planned), `phase/5-integration` (planned). Created from `main`. All work for a phase happens here. Merged to `main` via `--no-ff` (preserving the phase boundary) once approved. After merge, tagged and the branch may be deleted. Naming reflects the agentic pivot of 2026-04-27 — original `phase/4-projects` and `phase/5-release` are now backlog work and will get fresh phase names if/when revived.
...
- `v0.2.5-logexport` — end of Phase 2.5.
- `v0.3-history` — end of Phase 3.
- `v0.3.5-diagnostics` — end of Phase 3.5 (planned).
- `v0.4-agent` — end of Phase 4 (planned). Renamed from the original `v0.4-projects` after the agentic pivot of 2026-04-27.
- `v0.5-integration` — end of Phase 5 (planned). Renamed from the original `v1.0-release` — Phase 5 is now Android-deep integration, not public release.
- `v1.0-release` — first public release. Lands after the visual-identity-polish backlog is done; no longer tied to a specific phase number.
...
## Forthcoming — Phase 4 Agent layer (preview, refine in user-spec / tech-spec)

This section captures rules already decided for the agentic pivot (2026-04-27) — they will be enforced in code only when Phase 4 lands. Listed here so the Phase 4 user-spec / tech-spec inherits them without rediscovery.

**Three-tier tool consent.** Every Phase-4 tool must carry one of three consent tiers, applied uniformly across the agent:

| Tier | Trigger | Examples (illustrative — final list lives in Phase 4 user-spec) |
|---|---|---|
| **🟢 Green — auto-execute** | No confirmation. Read-only or trivially reversible side effect. | clipboard read, contact lookup, calendar read, currently-playing media query, notification snapshot |
| **🟡 Yellow — confirm** | Modal dialog showing tool name + arguments before execute. | system settings change, alarm/timer create, calendar event create, draft message in messenger (saved as draft, not sent) |
| **🔴 Red — confirm + rationale** | Modal dialog with arguments + agent's stated rationale (one sentence) + confirmation. | SMS / WhatsApp / Telegram send, phone call, file delete, irreversible third-party API write |

`ChatViewModel` (or its agent-mode equivalent) decides the tier based on tool metadata — never the model. Models cannot upgrade or downgrade their own tier. Autonomous (background-event-triggered) red tier is forbidden in Phase 4 — only allowed in Phase 4.5 with explicit per-rule pre-authorization stored on-device.

**Tools forbidden in quick chat.** Quick / incognito chat mode does not allow any tool — green tier included. Rationale: incognito's "no trace" promise breaks as soon as the agent does anything observable in third-party apps. Enforced by routing: `ChatIdentity.Quick` (or equivalent) must not be passed to the agent dispatcher; reviewers (manual + automated) check this.

**Airplane-safe constraint, first wave.** The first wave of tools shipping with Phase 4 must work without network. Pure on-device only: `ContactsContract`, `CalendarContract`, `Settings.System`, `AlarmClock.ACTION_SET_*`, clipboard, NotificationListenerService. Network-dependent tools (third-party REST APIs — Outlook Graph, Pachka, Singularity, Telegram Bot API, Yandex CalDAV) ship in a second wave once the airplane-safe wave is stable. Rationale: airplane-mode reliability is a use case in itself, and it forces a clean separation between "local primitives" and "cloud reach" that simplifies error handling.

**1/2/3 quality filter on every tool.** Every tool must score on at least one: (1) bypasses 5+ taps reliably vs OS path, (2) composes with other tools in a single user prompt, (3) uses context the user didn't type. Tools failing all three are rejected in user-spec review. Anti-pattern: Gallery's "turn flashlight on" tool, which loses to a Quick-Settings tile.

**Tools registry shape (placeholder, refine in tech-spec).** Tool *interface* (`Tool`, `ToolSchema`, `ToolArgs`, `ToolResult`) lives in `:core-runtime/tools/` — UI-free, no Android-specific dependencies in the interface itself. Tool *implementations* live in `:app/agent/tools/` — they may freely use Android APIs. `:app` registers concrete tools at startup via a `ToolRegistry` injected through Hilt, mirroring the `DefaultDownloadRepository.mainActivityFqn` cross-module plumbing pattern.

**ErrorLog components reserved.** When Phase 4 lands, `ErrorLog.ALLOWED_COMPONENTS` will gain at minimum: `tool-execute` (tool-call failures), `tool-permission` (consent / permission flow failures), `agent-route` (agent dispatcher errors). Phase 3.5 evaluated a `pre-flight-gate` component and rejected it — gate trips are preventive UX, not incidents (see "Pre-flight model-RAM gate" above). Updates to the whitelist follow the existing convention (constant set in `ErrorLog.kt` + this doc + the spec adding the component).
