...
- `ChatScreen` — TopAppBar has three actions: Settings (⚙ Tune), Reset (↻), Back. Body is the scrollable message list with streaming text + collapsible thinking block. Bottom is `MultimodalInputBar` (text field + camera + gallery + mic attach buttons; icons visible conditional on `Model.llmSupport*` flags — AC-18). `ThumbnailStrip` sits above the input bar when attachments exist (✕ on each to remove).
- `QuickChatScreen` — Phase 3+ (not yet built as of `v0.2-ui`). Visually identical to `ChatScreen` but with a clear "incognito" indicator and no "save" affordances.
- `ModelManagerScreen` — list of models from allowlist, each row showing download state (not downloaded / downloading N% / downloaded + size). TopAppBar has an Info icon that opens `AboutScreen` (per D17 Phase 2 tech-spec — entry-point placement is not prescribed, any screen may open About via `navigate("about")`).
- `InferenceSettingsBottomSheet` (per-model, NOT a separate screen) — opened from `ChatScreen` TopAppBar ⚙. Seven fields: `maxTokens`, `topK`, `topP`, `temperature`, `enableThinking` (hidden if `Model.llmSupportThinking=false`), `accelerator` (GPU/CPU), `systemPromptDefault`. Effective value = allowlist-default ∪ DataStore override. Buttons: **Применить** (saves override, triggers light/semi-light/heavy apply — see `patterns.md § Three-tier settings classification`), **Default** (clears all overrides for this model). Heavy fields route through `HeavyChangeDialog` + `ReinitProgressDialog`.
- `SettingsScreen` — Phase 5 (not built in Phase 2). Will host theme switch, log export, HuggingFace sign-in. Phase 2 deliberately does NOT create this screen (D5 Phase 2 user-spec — nothing yet to put in it).
- `AboutScreen` — scrollable, renders `app/src/main/assets/about.md` via `SafeMarkdown`. Footer: `BuildConfig.VERSION_NAME` + attribution to Google AI Edge Gallery, Gemma, LiteRT-LM. User edits the `.md` directly; rebuild → updated text.
