...
5. The last two waves are always **Audit Wave** and **Final Wave**, in that order:
   ...
   **Audit Wave** (always present) — 3 tasks running in parallel, `reviewers: none`:
   ...
   Auditors read all source files from the feature and write reports (analysis only). If issues found — a fix task is added with the appropriate skill and the original auditors as reviewers.
