...
#### Task 10: Test Audit
- **Description:** Full-feature test quality audit across `CrashHandlerTest`, `CrashStateTest`, `LogExportManagerTest`, `DeviceInfoCollectorTest`, `LogcatReaderTest`, `TapCounterTest`. Focus: behavioural coverage of user-spec AC (header fields, `[empty]` placeholders, section order, truncation, banner visibility logic, 7-tap counter), meaningful assertions beyond nullability, Robolectric consistency with `ErrorLogTest`, pyramid balance (size M → unit-only is correct; integration/E2E absence justified by `architecture.md § Testing`). Write report.
- **Skill:** test-master
- **Reviewers:** none
