...
4. AC-F3 is owned by `WarmupCoordinator` (not `SanctumApplication`): WarmupCoordinator already injects both `AppSettingsRepository` and `ModelRegistry`, so it launches a coroutine observing `registry.models` for any model transitioning to `isDownloaded=true` while `default_model_id` is empty → calls `appSettings.setDefaultModelId(modelId)`. `ModelEntry.isDownloaded` is kept current by `DefaultModelRegistry` (no Phase-3 changes to `DownloadWorker` needed — the existing Phase-2 registry update mechanism is sufficient).
...
- **Description:** Establish the Sanctum Machina visual foundation from the `design_handoff_phase3_ui` spec. Create `SanctumColors` data class with all 13 light + 13 dark tokens; wire into Material 3 `ColorScheme` overrides in `Theme.kt`. Add three font families (Cormorant Garamond, Inter, JetBrains Mono) via `androidx.compose.ui:ui-text-google-fonts`; define `SanctumTypography`. Set global shape theme: `2dp` corner radius for buttons/cards/chips, `22dp` pill for input bar. Add `SanctumIncognitoTheme` composable wrapper (dark palette regardless of system theme, applied to `chat/quick` route). Create `SmSigil` composable (diamond-in-diamond SVG sigil, accent-colored). Add all 21 custom icons as `ImageVector` constants in `SanctumIcons.kt`. All subsequent UI tasks (7–11) use these tokens — changing a token later updates the entire app.
- **Skill:** code-writing
- **Reviewers:** code-reviewer, test-reviewer
- **Verify-user:** Install APK → HomeScreen renders with warm parchment background, Cormorant Garamond title, sharp 2dp buttons; dark theme toggle shows dark palette; no default Material teal anywhere
- **Files to modify:** `app/src/main/kotlin/app/sanctum/machina/ui/theme/Theme.kt`, `app/src/main/kotlin/app/sanctum/machina/ui/theme/Color.kt`, `app/src/main/kotlin/app/sanctum/machina/ui/SanctumApp.kt`
- **Files to create:** `app/src/main/kotlin/app/sanctum/machina/ui/theme/SanctumColors.kt`, `app/src/main/kotlin/app/sanctum/machina/ui/theme/SanctumIcons.kt`, `app/src/main/kotlin/app/sanctum/machina/ui/theme/SanctumTypography.kt`, `app/src/main/kotlin/app/sanctum/machina/ui/theme/SanctumIncognitoTheme.kt`, `app/src/main/kotlin/app/sanctum/machina/ui/components/SmSigil.kt`
- **Files to read:** `Sanctum Machina design/design_handoff_phase3_ui/sanctum-tokens.jsx`, `Sanctum Machina design/design_handoff_phase3_ui/sanctum-atoms.jsx`, `Sanctum Machina design/design_handoff_phase3_ui/README.md`, `app/src/main/kotlin/app/sanctum/machina/ui/theme/Theme.kt`, `app/build.gradle.kts`
...
- **Description:** Create `WarmupCoordinator` (injects `ModelRegistry` + `AppSettingsRepository`): reads `AppSettings.default_model_id` (fallback `last_used_model_id`) → calls `registry.initialize(modelId)` in a cancellable coroutine → updates `last_used_model_id` on success → `ErrorLog.e("engine-warmup", ...)` on failure. Exposes `warmupDefault()` and `cancelAndRestart(modelId: String)`. Also owns AC-F3 observer: collects `registry.models` for any model becoming `isDownloaded=true`; if `appSettings.getDefaultModelId()` is empty at that moment → `appSettings.setDefaultModelId(modelId)`. Create `AppModule` providing `SanctumDatabase`, `ChatDao`, `MessageDao`, `ChatRepository`, and `WarmupCoordinator`. Unit-test: model resolution order (a/b/c per AC-F5), `cancelAndRestart` racing, AC-F3 observer triggers correctly.
- **Skill:** code-writing
- **Reviewers:** code-reviewer, security-auditor, test-reviewer
...
- **Description:** Extend `SanctumApplication.onCreate` (behind `getProcessName() == packageName` guard, after `installCrashHandler()`): inject `WarmupCoordinator` + `ChatRepository` via `@Inject`; call `warmupCoordinator.warmupDefault()` (WarmupCoordinator also handles AC-F3 observer — no additional injection of `AppSettingsRepository` in SanctumApplication needed); trigger `SettingsMigrationHelper.migrateIfNeeded()`. Run housekeeping coroutines with error handling: `filesDir/quick/` purge wrapped in `try/catch + ErrorLog.e("attachment-save", ...)` (failure breaks quick-chat incognito guarantee — must not be silent); orphan `.staging-*` cleanup with same error handling; `chatRepository.sweepZombieChats()`. Wrap `Room.databaseBuilder.build()` in AppModule with corruption handler: on exception, rename `sanctum.db` to `sanctum.db.corrupt_{ts}`, create fresh DB, expose `corruptionOccurred` via `AppCorruptionState` singleton, log `ErrorLog.e("history-read", ...)`.
- **Skill:** code-writing
- **Reviewers:** code-reviewer, security-auditor, test-reviewer
...
- **Files to read:** `app/src/main/kotlin/app/sanctum/machina/ui/SanctumApp.kt`, `app/src/main/kotlin/app/sanctum/machina/ui/modelmanager/ModelManagerScreen.kt`, `core-runtime/src/main/kotlin/app/sanctum/machina/core/registry/ModelRegistry.kt`, `work/phase-3-history/tech-spec.md` (Data Models → ChatIdentity, AC-P3, AC-P5, US-8), `Sanctum Machina design/design_handoff_phase3_ui/sanctum-screen-home.jsx`, `Sanctum Machina design/design_handoff_phase3_ui/sanctum-screen-drawer.jsx`
...
- **Files to read:** `app/src/main/kotlin/app/sanctum/machina/ui/chat/ChatScreen.kt`, `app/src/main/kotlin/app/sanctum/machina/ui/chat/ChatViewModel.kt`, `app/src/main/kotlin/app/sanctum/machina/engine/AppCorruptionState.kt`, `app/src/main/kotlin/app/sanctum/machina/logexport/LogExportManager.kt`, `work/phase-3-history/tech-spec.md` (AC-E3, AC-E3b, AC-U4–U7, US-2-bis, US-4, US-9, US-14), `Sanctum Machina design/design_handoff_phase3_ui/sanctum-screen-chat.jsx`, `Sanctum Machina design/design_handoff_phase3_ui/sanctum-screen-quick.jsx`, `Sanctum Machina design/design_handoff_phase3_ui/sanctum-screen-more.jsx`
...
### Wave 5.5 (зависит от Task 4, Task 6, Task 8 — идёт параллельно с Wave 5 или до Task 10)

#### Task 17: Attachment staging (Draft→Persistent file atomicity)
- **Description:** Закрывает Phase-3 debt 2 (Task 8 отклонение). Добавляет атомарную модель сохранения вложений для Draft→Persistent перехода и прямую запись для уже-созданных Persistent-чатов. Реализуется по варианту staging (tech-spec Decision 6): в момент `addImages`/`addAudio`/`addImageBitmap` VM lazy-создаёт `filesDir/attachments/.staging-{uuid}/` и пишет сериализованный `Attachment` (PNG для `Attachment.Image`, WAV для `Attachment.Audio` — тот же формат, что уже оборачивается в `pcmToWav` для передачи в litertlm) на `Dispatchers.IO` через новый `ChatRepository.writeAttachmentStaging(...)`. `removeAttachment` удаляет файл на диске вместе со снятием из `_attachments`. `commitDraft` передаёт этот `stagingDir` в уже существующий `chatRepository.commitDraftChat(modelId, firstMessage, stagingDir, filesDir)` — атомарный rename в `attachments/{chatId}/` внутри Room-транзакции (код уже реализован в Task 4). Поля `imagePath`/`audioPath` в `MessageEntity` заполняются финальными относительными путями **внутри** транзакции, так что Room-строка и диск всегда согласованы. Для Persistent-режима (последующие сообщения в уже-созданном чате) staging не нужен — вводится отдельный метод `chatRepository.savePersistentAttachment(chatId, attachment): Pair<String?, String?>` (image-path, audio-path), который пишет прямо в `filesDir/attachments/{chatId}/`. Поскольку `chatId` уже в Room, atomicity не требуется — partial-fail лечится startup-sweep'ом. `SanctumApplication` housekeeping уже удаляет orphan `.staging-*` (Task 6 AC-A3) — проверить, что контракт соблюдается после добавления staging-writes.
- **Skill:** code-writing
- **Reviewers:** code-reviewer, security-auditor, test-reviewer
- **Verify-smoke:** `./gradlew :app:testDebugUnitTest --tests "*.ChatRepositoryTest" --tests "*.ChatViewModelTest"` → зелёные; `ls app/src/main/kotlin/app/sanctum/machina/data/DefaultChatRepository.kt` grep для `writeAttachmentStaging\|savePersistentAttachment` — обе сигнатуры присутствуют
- **Verify-user:** открыть Draft → прикрепить картинку → нажать Send → после перехода в Persistent-чат картинка видна в истории; перезапустить приложение (kill процесса) → картинка остаётся на месте. Прикрепить картинку в Draft → `removeAttachment` — файл исчез с диска (`ls filesDir/attachments/.staging-*` пусто). Прикрепить картинку в Draft → свернуть приложение → system kill → перезапустить → staging убран startup-sweep'ом (AC-A3).
- **Files to modify:** `app/src/main/kotlin/app/sanctum/machina/data/ChatRepository.kt`, `app/src/main/kotlin/app/sanctum/machina/data/DefaultChatRepository.kt`, `app/src/main/kotlin/app/sanctum/machina/ui/chat/ChatViewModel.kt`, `app/src/test/kotlin/app/sanctum/machina/data/ChatRepositoryTest.kt`, `app/src/test/kotlin/app/sanctum/machina/ui/chat/ChatViewModelTest.kt`, `app/src/main/kotlin/app/sanctum/machina/data/model/MessageEntity.kt` (*если* требуется helper для relative-path normalization — предпочтительно держать в util)
- **Files to read:** `app/src/main/kotlin/app/sanctum/machina/data/DefaultChatRepository.kt` (текущий commitDraftChat), `app/src/main/kotlin/app/sanctum/machina/data/dao/MessageDao.kt`, `app/src/main/kotlin/app/sanctum/machina/ui/chat/ChatViewModel.kt` (addImages/addImageBitmap/addAudio/removeAttachment/commitDraft/runInferencePersistent), `app/src/main/kotlin/app/sanctum/machina/ui/chat/Attachment.kt`, `core-runtime/src/main/kotlin/app/sanctum/machina/core/common/PcmToWav.kt`, `app/src/main/kotlin/app/sanctum/machina/SanctumApplication.kt` (orphan staging cleanup), `work/phase-3-history/tech-spec.md` (Decision 6, AC-A1, AC-A3, AC-A6)
