...
10. Печатаю "Что на фото?" → Send. Bitmap'ы пакуются в `List<Bitmap>`, уходят в `helper.runInference`. В пузыре отправленного USER-сообщения сразу показываются миниатюры прикреплённых фото над текстом (и chip с длительностью для аудио, когда AC-12 появится), чтобы история оставалась консистентной с отправленным контентом. Приходит стримящийся ответ, автоскролл тянет ленту к последнему сообщению. Markdown рендерится через compose-richtext.
...
- [ ] **AC-22.** Финальный гейт Phase 2: AC-1..21, AC-26 выполнены; регрессии Phase 1 (AC-16) отсутствуют; пользователь явно проставил апрув на Honor 200 после ручного прохождения US-1..US-7. Численный гейт TTFT/latency отдельно не замеряется (это не валидационная фаза) — достаточно AC-16 backstop'а.
...
### Дополнительно обязательные (обнаружены на user-verify)

- [ ] **AC-26.** В истории чата USER-сообщения отрисовывают все прикреплённые вложения: фото — миниатюрами над текстом, аудио — chip с длительностью. История остаётся консистентной с отправленным контентом (пользователь видит, что именно он отправил модели). Вложения живут в памяти — эфемерно, как и сам чат в Phase 2 (D1 user-spec); Phase 3 (Room) перейдёт на сериализованные пути / file-refs без изменения UX.
