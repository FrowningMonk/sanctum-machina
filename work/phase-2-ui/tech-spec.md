...
- **Полу-лёгкие** (`systemPromptDefault`, `enableThinking`): применяются через `registry.resetConversation(modelName, systemPrompt = effective)` — engine не переинициализируется, но KV-кеш сбрасывается. Следующий `send()` рендерит Jinja-шаблон с обновлёнными значениями (`extraContext = mapOf("enable_thinking" to "true")` для thinking, новый префикс контента для systemPrompt). UX: snackbar «Настройки применены, контекст чата сброшен». Чат-лента в UI **очищается**.
- **Тяжёлые** (требуют `cleanup + initialize`): только `accelerator`. `HeavyChangeDialog` + `ReinitProgressDialog`.

`enableThinking` изначально классифицировался как heavy provisional. Honor 200 smoke (Task 11, 2026-04-18) подтвердил, что litertlm 0.10.0 принимает смену `enable_thinking` через `resetConversation` без полной reinit — флаг влияет только на рендеринг следующего turn'а в Jinja-шаблоне, а KV-кеш предыдущих ходов чистится reset'ом. Перенесён в полу-лёгкие; результат зафиксирован в `decisions.md` Task 11.
**Rationale:** user-spec AC-21 явно говорит про `systemPromptDefault` = «next `resetConversation`»; это не «на следующий send», но и не reinit. `enableThinking` оказался изоморфен по природе изменения (template-level, не engine-level).
