...
### D24. `systemPromptDefault` plumbing — `ConfigKey` + `createLlmChatConfigs` + engine systemInstruction
**Decision:** Добавить в `:core-runtime`:
1. `ConfigKeys.SYSTEM_PROMPT_DEFAULT = ConfigKey("systemPromptDefault")` (уже есть в коде согласно code-research; если нет — создать).
2. `createLlmChatConfigs(..., defaultSystemPrompt: String = "")` — эмитит `LabelConfig(SYSTEM_PROMPT_DEFAULT, defaultValue = defaultSystemPrompt)` в список config'ов (попадает в `Model.configValues`).
3. `AllowedModel.toModel()` — прокидывает `defaultSystemPrompt` из allowlist JSON (поле `defaultConfig.systemPromptDefault`, optional; default `""`) в `createLlmChatConfigs`.
4. `LlmChatModelHelper.initialize(...)` — `model.configValues[ConfigKeys.SYSTEM_PROMPT_DEFAULT.label] as? String` → оборачивается в `Contents.of(listOf(Content.Text(it)))` → передаётся `engine.createConversation(systemInstruction = ...)` (вместо текущего `null`).
5. `LlmChatModelHelper.resetConversation(model, systemInstruction = ...)` — уже принимает systemInstruction (Phase 1 API); `ChatViewModel.resetConversation` формирует `Contents` из effective systemPrompt, передаёт.
...
class SafeUriHandler(private val context: Context) : UriHandler {
  override fun openUri(uri: String) {
    ...
    if (parsed.scheme in setOf("http", "https")) {
      ...
      // silently ignore; optionally ErrorLog.e("safe-uri", "blocked scheme: ${parsed.scheme}")
    }
  }
}
```
...
### D26. Privacy hardening — `FLAG_SECURE`, `allowBackup=false`, `dataExtractionRules`
**Decision:**
1. `MainActivity.onCreate` перед `setContent`: `window.setFlags(FLAG_SECURE, FLAG_SECURE)` — блокирует recents-screen snapshot и screenshots (чат-контент не утекает через overview-экран Android).
2. `<application android:allowBackup="false" ... />` — Google auto-backup отключён (DataStore-настройки и Phase-3 Room не передаются через cloud).
3. `<application android:dataExtractionRules="@xml/data_extraction_rules" ... />` (Android 12+) — пустой allowlist в `res/xml/data_extraction_rules.xml`, блокирует device-to-device transfer.

**Rationale:** Sanctum Machina — local-LLM privacy app (project.md § Out of Scope: «no cloud sync»). Defaults хуже: backup включён, recents содержит снимок чата. Стоимость фикса — три строки, покрывает оба вектора. Должно быть в Phase 2 — в Phase 3 появится Room-история, и упущение станет дорогостоящим.
**Alternatives considered:** per-view `FLAG_SECURE` на ChatScreen only — overengineering, проще на Activity целиком.
- Supports: Security (A04 Insecure Design, M2 Inadequate Privacy Controls), user-spec manifesto (project.md «no cloud sync»).
...
- Supports: R13, completeness F-05.
...
```kotlin
object ConfigKeys {
  // existing: MAX_TOKENS, TOPK, TOPP, TEMPERATURE, ENABLE_THINKING, ACCELERATOR, VISION_ACCELERATOR, SUPPORT_IMAGE, SUPPORT_AUDIO, THEME
  val SYSTEM_PROMPT_DEFAULT = ConfigKey("systemPromptDefault")   // NEW (verify existence in Phase-1 code; if already present, no-op)
}
...
fun createLlmChatConfigs(
  ...
  defaultSystemPrompt: String = "",   // NEW param
): List<Config> = buildList {
  // existing: MaxTokens, TopK, TopP, Temperature, Accelerator, EnableThinking
  add(LabelConfig(ConfigKeys.SYSTEM_PROMPT_DEFAULT, defaultSystemPrompt))   // NEW
}
```

### `:core-runtime` engine systemPrompt wiring

`inference/LlmChatModelHelper.kt::initialize`:
```kotlin
...
engine.createConversation(systemInstruction = systemInstruction, ...)  // was: systemInstruction = null
```
...
| R15. `FLAG_SECURE` ломает screenshots в dev-builds | `MainActivity`: `if (BuildConfig.DEBUG) window.clearFlags(FLAG_SECURE) else setFlags(FLAG_SECURE)` — dev можно скриншотить, release защищён (debate — в комментариях к D26; initial: всегда `setFlags`) |
...
### 9. Privacy hardening — `FLAG_SECURE`, `allowBackup=false`, `dataExtractionRules`
- **Не в user-spec:** D26 добавляет три privacy-умолчания. user-spec явно не требует, но project.md манифест «no cloud sync» и характер local-LLM privacy app делают это sane defaults. Без них backup чат-контента через Google включён, recents-screen содержит snapshot чата. Стоимость — 3 строки в коде. В Phase 3 (Room) упущение становится дорогостоящим. → **[PENDING USER APPROVAL]**

### 10. Markdown link-scheme whitelist (`SafeUriHandler`)
- **Не в user-spec:** D25 добавляет scheme-whitelist для link-handler markdown. user-spec явно не требует, но LLM-output рендерится markdown, и `[text](intent://...)` — валидный attack vector. → **[PENDING USER APPROVAL]**

### 11. `ErrorLog` component whitelist расширение
- **Не в user-spec:** D27 добавляет `"settings-io"`, `"camera"`, `"audio"`, `"attachment-decode"` в closed whitelist `patterns.md § ErrorLog`. user-spec явно не требует, но patterns.md Phase 1 рассматривает whitelist как closed — без расширения логировать Phase-2-failure-paths негде. → **[PENDING USER APPROVAL]** (требует правки `patterns.md`).
...
- [ ] **TAC-8.** `rg -l "androidx.compose|androidx.activity" core-settings/src/main` → пусто.
- [ ] **TAC-9.** APK собран, `aapt dump permissions` содержит CAMERA + RECORD_AUDIO.
- [ ] **TAC-10.** `aapt dump xmltree app-debug.apk AndroidManifest.xml | grep allowBackup` показывает `allowBackup=false`.
- [ ] **TAC-11.** `rg -n "Text\(\"[А-Яа-я]" app/src/main` → пусто (все строки через `stringResource`).
- [ ] **TAC-12.** Generated `AppSettings`, `PerModelSettings` в `:core-settings/build/generated/...`, `hasMaxTokens()`/`getMaxTokens()` методы присутствуют.
- [ ] **TAC-13.** `SafeUriHandler.openUri` игнорирует `intent://test`, `sms:test`, `tel:test` — добавить в `ChatViewModelTest` или отдельный `SafeUriHandlerTest`.
...
- **Verify-user:** APK на Honor 200 → launcher-label «Sanctum Machina» (AC-2); overview/recents экран при открытом ChatScreen показывает чёрный/blank snapshot вместо чата (FLAG_SECURE).
- **Files to modify:** `app/src/main/res/values/strings.xml`, `app/src/main/AndroidManifest.xml`, `app/build.gradle.kts`, `gradle/libs.versions.toml`, `app/src/main/kotlin/app/sanctum/machina/MainActivity.kt`
...
- **Description:** Acceptance testing. Запустить: `./gradlew build`, `:core-runtime:test`, `:core-settings:test`, `:app:test`, `lintDebug`, `:app:assembleDebug`. Verify TAC-1..TAC-13. Маппинг user-spec AC-1..AC-22 на автоматически проверяемое / user-verify-only. AC-2, AC-13, AC-14, AC-16, US-1..US-7 — deferred to user on Honor 200. AC-23 (audio duration) автоматически через Attachment.Audio; AC-24 (peak amplitude) — manual-smoke с визуальной проверкой индикатора; AC-25 (multi-line thumbnail) — помечено implement/defer. Финальный гейт AC-22 — user approval на устройстве.
- **Skill:** pre-deploy-qa
- **Reviewers:** none
