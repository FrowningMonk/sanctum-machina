...
- **Draft-attachment persistence — закрывается Task 17.** Task 8 передаёт `stagingDir = null` в `commitDraftChat` и не сохраняет вложения, прикреплённые в Draft. Task 17 добавляет `writeAttachmentStaging` + `savePersistentAttachment` в `ChatRepository`, вводит lazy `draftStagingDir` в VM и замыкает путь attachment → диск → Room-transaction-rename → `attachments/{chatId}/`. См. `tasks/17.md`.
