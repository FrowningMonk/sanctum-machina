...
- User: смоук на Honor 200 по AC из `tasks/17.md` → Verification Steps → User (Draft+картинка → Send → история сохраняет вложение; kill-and-reopen проверка персистентности; cancel в Draft удаляет staging файл; orphan cleanup; Persistent+картинка) — **pending**
