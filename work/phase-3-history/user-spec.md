...
### US-4. Открытие старого чата на другой модели (read-first)

1. Drawer → тап на чате; `model_id` ≠ активная. **Навигация происходит сразу**, без диалога.
2. ChatScreen открывается мгновенно, вся история из Room отображается. Пользователь может просто перечитать старый разговор.
3. TopAppBar имя модели показано как кнопка **«Загрузить»** (AC-U6 паттерн) — сигнализирует, что движок для этой модели не готов. Send disabled, набирать текст тоже можно (текст пока никуда не уйдёт).
4. Если пользователь ушёл (Back) не отправив ничего — активная модель не тронута, никакого warmup'а не запустилось. **Бесплатное чтение.**
5. Если хочет продолжить → тап «Загрузить» в TopAppBar → `cleanup(active) + initialize(chat.model_id)` в фоне, TopAppBar меняется на спиннер + «Модель перезагружается…». Send остаётся disabled.
6. По завершении reinit → TopAppBar становится read-only label с именем модели, Send активен, разговор продолжается.
7. Если warmup упал → TopAppBar снова «Загрузить» (AC-U6 failed-state), Snackbar с причиной. Повторный тап — retry.
...
### US-8. Первый запуск без скачанных моделей и выбор default-модели
...
4. Пользователь переходит в Model Manager → скачивает модель → она автоматически становится default (`AppSettings.default_model_id = modelId`) → главный экран превращается в обычный.
5. **Правила выбора default при warmup** (`SanctumApplication.onCreate`):
   - **(a)** `AppSettings.default_model_id` задан **и** файл модели существует → прогреваем её.
   - **(b)** Иначе `AppSettings.last_used_model_id` задан **и** файл существует → прогреваем её (fallback).
   - **(c)** Иначе → warmup не запускается; main screen — placeholder (шаг 2).
6. `last_used_model_id` автоматически обновляется при каждом успешном `registry.initialize(modelId)`.
7. **Смена default пользователем:** в Model Manager у каждой скачанной модели в overflow-меню строки (⋮) — пункт **«Сделать по умолчанию»**. Текущий default помечен ⭐ слева от имени. Тап «Сделать по умолчанию» → `AppSettings.default_model_id = modelId` → Snackbar «Модель по умолчанию: {name}». При следующем cold start прогревается уже она.
...
- [ ] **AC-E3.** При переходе на чат с `model_id != активная` — навигация **без диалога**, чат открывается read-only (Send disabled), TopAppBar показывает «Загрузить» (AC-U6). Warmup запускается только при явном тапе «Загрузить», не автоматически.
- [ ] **AC-E3b.** Cross-model confirmation dialog **остаётся** только для перехода модели в draft-чате через TopAppBar model-picker (AC-U7). Там warmup тяжёлый и явный — подтверждение сохраняется.
...
- [ ] **AC-F3.** После первого успешного скачивания любой модели она автоматически становится `default_model_id` (если `default_model_id` пустой).
- [ ] **AC-F4.** Model Manager «Загрузить» открывает новый quick chat на этой модели (без pre-nav dialog; если другая уже прогрета, reinit запустится при открытии quick-чата тем же in-chat Load-паттерном что и AC-U6).
- [ ] **AC-F5.** `SanctumApplication.onCreate` warmup порядок: (a) default_model_id если задан и файл есть → прогрев; (b) иначе last_used_model_id → прогрев; (c) иначе warmup не запускается.
- [ ] **AC-F6.** `last_used_model_id` обновляется при каждом успешном `registry.initialize(modelId)`.
- [ ] **AC-F7.** Model Manager: каждая строка скачанной модели имеет в overflow-меню пункт «Сделать по умолчанию». Текущий default помечен ⭐. Тап → `AppSettings.default_model_id = modelId` + Snackbar.
...
- **Мы решили** НЕ показывать pre-navigation dialog при открытии чата на другой модели, а открывать чат сразу read-only с кнопкой «Загрузить» в TopAppBar. Это даёт бесплатное чтение старых разговоров и явный intent для warmup'а только когда пользователь реально хочет продолжить.
- **Мы решили** размещать переключение default-модели в overflow-меню строки Model Manager, а не в новом Settings-экране, потому что Settings screen не оправдан одной dropdown'ой (Phase 2 D5 прецедент).
...
- **AC-E3 + US-4 (read-first cross-model):** открою чат на E2B (пока активна E4B) → ChatScreen открывается сразу без диалога, история видна, TopAppBar показывает «Загрузить», Send disabled → уйду Back — активная модель не менялась. Открою ещё раз → тапну «Загрузить» в TopAppBar → спиннер + «Модель перезагружается…» → через 20–30 сек Send активен.
- **AC-F7 (default model switch):** в Model Manager на строке неактивной модели тапну overflow → «Сделать по умолчанию» → Snackbar подтверждения, ⭐ переехала на неё. Закрою приложение, открою снова → в фоне прогревается выбранная.
