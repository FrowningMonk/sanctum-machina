...
- [ ] **AC-A6.** Multi-attachment atomicity: при отправке сообщения с N вложениями все N файлов сначала пишутся в staging-директорию `filesDir/attachments/.staging-{uuid}/`, затем вся директория атомарно переименовывается в `filesDir/attachments/{chatId}/`. Если на k-ом файле (k<N) падает IOException → всю staging-директорию `deleteRecursively`, в `filesDir/attachments/{chatId}/` ничего не появляется, Room rows не создаются. Orphan staging-директории (`.staging-*`) подбираются и удаляются в `SanctumApplication.onCreate`.
...
- [ ] **AC-U6.** TopAppBar recovery-load: когда движок не готов (warmup идёт / warmup упал / init упал) — имя модели превращается в кнопку «Загрузить»; тап → `registry.initialize(modelId)` повторяет warmup. Когда движок готов — label informational.
- [ ] **AC-U7.** Draft chat: TopAppBar имя модели — кликабельный model-picker (dropdown скачанных моделей). После первого Send — read-only label.
